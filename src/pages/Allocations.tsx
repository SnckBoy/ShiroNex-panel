...
export default function Allocations() {
  ...
  const nodeServers = servers.filter((server) => String(server.nodeId || "") === String(nodeId));
  const selectedNode = nodes.find((node) => node.id === nodeId);
  const allocatedPorts = visibleAllocations.reduce((total, allocation) => total + Math.max(0, allocation.portEnd - allocation.portStart + 1), 0);
  const assignedPorts = visibleAllocations.filter((allocation) => allocation.assignedServerId).reduce((total, allocation) => total + Math.max(0, allocation.portEnd - allocation.portStart + 1), 0);

  const add = async () => {
    if (!nodeId) return setError("Select a node first.");
    if (!form.ip.trim()) return setError("Enter an IP address.");
    const start = Number(form.portStart);
    const end = Number(form.portEnd);
    if (!Number.isInteger(start) || !Number.isInteger(end) || start < 1 || end > 65535) return setError("Enter ports between 1 and 65535.");
    if (start > end) return setError("Start port must be less than or equal to end port.");
    if (end - start + 1 > 10000) return setError("A single range can contain at most 10,000 ports.");
    setBusy(true);
    try { await axios.post("/api/allocations", { nodeId, ...form, portStart: start, portEnd: end }); setForm((current) => ({ ...current, ip: "", primary: false })); await load(); }
  ...
  return <div className="mx-auto max-w-6xl p-4 md:p-6">
    ...
    <section className="snx-console-surface mb-6 rounded-2xl p-4 md:p-5"><div className="mb-4 flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm font-semibold text-foreground">Create allocation</p><p className="text-xs text-muted-foreground">Enter a start and end port; every port in the range is created as an independent allocation.</p></div>{selectedNode && <div className="flex gap-2 text-xs"><span className="rounded-full border border-cyan-300/20 bg-cyan-300/10 px-2.5 py-1 text-cyan-200">{selectedNode.name}</span><span className="rounded-full border border-white/10 px-2.5 py-1 text-muted-foreground">{assignedPorts}/{allocatedPorts || 0} ports assigned</span></div>}</div><div className="grid gap-3 md:grid-cols-6"><select value={nodeId} onChange={(event) => { setNodeId(event.target.value); setError(""); }} className="rounded-xl border border-border bg-background p-3 text-sm"><option value="">Select node</option>{nodes.map((node) => <option value={node.id} key={node.id}>{node.name} · {node.status || "unknown"}</option>)}</select><input required placeholder="IP address" value={form.ip} onChange={(event) => setForm({ ...form, ip: event.target.value })} className="rounded-xl border border-border bg-background p-3 text-sm" /><input required type="number" min="1" max="65535" placeholder="Start port" value={form.portStart} onChange={(event) => setForm({ ...form, portStart: event.target.value })} className="rounded-xl border border-border bg-background p-3 text-sm" /><input required type="number" min="1" max="65535" placeholder="End port" value={form.portEnd} onChange={(event) => setForm({ ...form, portEnd: event.target.value })} className="rounded-xl border border-border bg-background p-3 text-sm" /><select value={form.ipVersion} onChange={(event) => setForm({ ...form, ipVersion: event.target.value as "IPv4" | "IPv6" })} className="rounded-xl border border-border bg-background p-3 text-sm"><option value="IPv4">IPv4</option><option value="IPv6">IPv6</option></select><button type="button" disabled={busy} onClick={() => void add()} className="snx-primary-button"><Plus className="h-4 w-4" /> Add allocation</button></div><p className="mt-3 text-xs text-cyan-200/80">{Number(form.portStart) > 0 && Number(form.portEnd) >= Number(form.portStart) ? `${Number(form.portEnd) - Number(form.portStart) + 1} individual port allocation${Number(form.portEnd) - Number(form.portStart) + 1 === 1 ? "" : "s"} will be created.` : "Enter a valid port range to preview the number of allocations."}</p><label className="mt-3 inline-flex items-center gap-2 text-xs text-muted-foreground"><input type="checkbox" checked={form.primary} onChange={(event) => setForm({ ...form, primary: event.target.checked })} /> Mark as primary allocation</label></section>
