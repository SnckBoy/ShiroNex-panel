...
app.get("/v1/servers/:id/stats",auth,async(req,res)=>{try{const c=find(req.params.id),i=await c.inspect();if(!i.State.Running)return res.json({cpu:0,ram:0});const x:any=await c.stats({stream:false});const cpuDelta=x.cpu_stats.cpu_usage.total_usage-x.precpu_stats.cpu_usage.total_usage;const sysDelta=x.cpu_stats.system_cpu_usage-x.precpu_stats.system_cpu_usage;const cpus=x.cpu_stats.online_cpus||1;const cache=x.memory_stats.stats?.cache||x.memory_stats.stats?.inactive_file||0;res.json({cpu:sysDelta>0?cpuDelta/sysDelta*cpus*100:0,ram:Math.max(0,(x.memory_stats.usage-cache)/1024/1024)})}catch(e:any){res.status(502).json({error:e.message})}});
