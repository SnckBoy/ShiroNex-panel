...
<main class="mx-auto flex min-h-screen max-w-md items-center px-6 py-12">
    <section class="w-full rounded-2xl border border-slate-800 bg-slate-900/90 p-8 shadow-2xl">
        ...
        <p class="mt-2 text-sm text-slate-400">Sign in with your username or email.</p>
        @if ($errors->any())
        ...
        <form method="POST" action="{{ route('login.store') }}" class="mt-7 space-y-5">
            @csrf
            <label class="block text-sm">Username or email<input name="identity" value="{{ old('identity') }}" required autofocus class="mt-2 w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2.5 outline-none focus:border-cyan-400"></label>
            ...
            <button class="w-full rounded-lg bg-cyan-400 px-4 py-3 font-semibold text-slate-950 transition hover:bg-cyan-300">Sign in</button>
        </form>
    </section>
</main>
</body>
